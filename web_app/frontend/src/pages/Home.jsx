...
export default function Home() {
  ...
  const getAllGames = (
    ...
    filters_genre.forEach((filter) => {
      query += `genres=${filter}&`;
    });
    filters_platform.forEach((filter) => {
      query += `platforms=${filter}&`;
    });
  ...
  useEffect(() => {
    getAllGames(currPage, genres, platforms, sort), getFilters();
          ...
          <ReactPaginate
            ...
            nextLabel={
              <img
                src={NextArrow}
                className={`next-btn ${
                  currPage === allGames.info.total_pages ? "hidden" : "active"
                }`}
              />
            }
            previousLabel={
              <img
                src={PrevArrow}
                className={`prev-btn ${currPage === 1 ? "hidden" : "active"}`}
              />
            }
          />
        </div>
